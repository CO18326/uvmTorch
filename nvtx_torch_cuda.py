...
# Allocate memory
# ----------------------------
# Pinned host memory enables async HtoD / DtoH
...
# ----------------------------
def cpu_compute(ms):
    torch.cuda.nvtx.range_push("CPU Compute")
    t0 = time.time()
...
# ----------------------------
def gpu_compute(t):
    torch.cuda.nvtx.range_push("GPU Compute")
    for _ in range(10):
...
for i in range(ITERATIONS):
    ...
    with torch.cuda.stream(stream):
        # HtoD memcpy
        torch.cuda.nvtx.range_push("HtoD memcpy")
        ...
        # DtoH memcpy
        torch.cuda.nvtx.range_push("DtoH memcpy")
